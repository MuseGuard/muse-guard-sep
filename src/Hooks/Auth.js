...
const useLogin = () => {
  ...
  const login = async (password) => {
    ...
    try {
      const response = await axios.post('http://35.186.215.128/jwt/login', { password } , {mode : 'cors'});
  ...
  const updatePassword = async (oldPassword, newPassword) => {
    ...
    try {
      const response = await axios.patch('https://35.186.215.128/jwt/update-password', {
        oldPassword,
        newPassword,
      });
