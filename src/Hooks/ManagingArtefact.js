...
const ManagingArtefact = () => {
  ...
  const url = "http://34.88.237.151/artifacts/getAllArtifacts";
  ...
  return {
    ...
    handleArtefact: () => {
      axios
        .post("http://34.88.237.151/artifacts/postArtifact", artefact)
        .then((response) => {
    ...
    handleDeleteArtefact: (name) => {
      axios
        .delete(`http://34.88.237.151/artifacts/deleteart/${name}`)
        .then((response) => {
          console.log(response.data);
